...
-- for the POOL rule
module Test.Cardano.Ledger.Constrained.Babel.Pool where
...
poolEnvSpec ::
  IsConwayUniv fn =>
  Specification fn (PoolEnv (BabelEra StandardCrypto))
poolEnvSpec =
  constrained $ \pe ->
    match pe $ \_ pp ->
...
pStateSpec ::
  IsConwayUniv fn =>
  Specification fn (PState (BabelEra StandardCrypto))
...
poolCertSpec ::
  IsConwayUniv fn =>
  PoolEnv (BabelEra StandardCrypto) ->
  PState (BabelEra StandardCrypto) ->
