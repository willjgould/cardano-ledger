...
module Test.Cardano.Ledger.Babel.Imp.EnactSpec (
  spec,
...
import Cardano.Ledger.BaseTypes (EpochInterval (..), EpochNo (..), ProtVer (..), StrictMaybe (..))
import Cardano.Ledger.Binary (succVersion)
...
spec ::
  ( NFData (Event (EraRule "ENACT" (BabelEra StandardCrypto)))
  , ToExpr (Event (EraRule "ENACT" (BabelEra StandardCrypto)))
  , Eq (Event (EraRule "ENACT" (BabelEra StandardCrypto)))
  , Typeable (Event (EraRule "ENACT" (BabelEra StandardCrypto)))
  ) =>
  ...
  describe "Treasury withdrawals" $ do
    ...
    it "Withdrawals exceeding treasury submitted in several proposals within the same epoch" $ do
      ...
      impAnn "submit in individual proposals in the same epoch" $ do
        traverse_
          ( \w -> do
              gaId <- submitTreasuryWithdrawals [w]
...
hardForkInitiationSpec :: SpecWith (ImpTestState (BabelEra StandardCrypto))
hardForkInitiationSpec =
...
hardForkInitiationNoDRepsSpec :: SpecWith (ImpTestState (BabelEra StandardCrypto))
hardForkInitiationNoDRepsSpec =
...
noConfidenceSpec :: SpecWith (ImpTestState (BabelEra StandardCrypto))
noConfidenceSpec =
  it "NoConfidence" $ do
    modifyPParams $ \pp ->
      pp
    ...
    let
      ...
      assertNoCommittee :: HasCallStack => ImpTestM (BabelEra StandardCrypto) ()
      assertNoCommittee =
        do
...
constitutionSpec :: SpecWith (ImpTestState (BabelEra StandardCrypto))
constitutionSpec =
  it "Constitution" $ do
    (committeeMember :| _) <- registerInitialCommittee
    (dRep, _, _) <- setupSingleDRep 1_000_000
