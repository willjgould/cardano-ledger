...
spec = describe "Regression" $ do
  it "DeserialiseFailure on resubmitting Babel Tx with invalid plutus script #4198" $ do
    io . expectRightDeep_ $
      ...
      it "InsufficientCollateral is not encoded with negative coin #4198" $ do
        ...
        let
          hasInsufficientCollateral
            (SwapsFailure (BabelUtxowFailure (UtxoFailure (InsufficientCollateral _ _)))) = True
