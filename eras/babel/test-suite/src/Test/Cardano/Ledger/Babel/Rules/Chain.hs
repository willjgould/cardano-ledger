...
module Test.Cardano.Ledger.Babel.Rules.Chain where
...
-- type instance EraRule "TICKN" (BabelEra c) = TICKN
...
--   Nonce ->
--   ChainState era
-- initialBabelState lab e utxo reserves genDelegs pp initNonce =
--   ChainState
--     ( NewEpochState
--         e
...
--             TRC
--               ( TicknEnv
--                   NeutralNonce -- (pp' ^. ppExtraEntropyL)
--                   etaC
--                   etaPH
--               , TicknState eta0 etaH
--               , e1 /= e2
...
--             TRC
--               ( PrtclEnv
--                   (unsafeBoundRational 0.5) -- minBound -- (pp' ^. ppDL)
--                   _pd
--                   genDelegs
--                   eta0'
--               , PrtclState cs etaV etaC
...
--   , Era era
--   , STS (BabelBBODY era)
--   , PredicateFailure (EraRule "BBODY" era) ~ BabelBbodyPredFailure era
--   , Event (EraRule "BBODY" era) ~ Event (BabelBBODY era)
--   ) =>
--   Embed (BabelBBODY era) (CHAIN era)
--   where
